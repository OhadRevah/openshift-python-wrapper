...
def custom_resource_definitions(admin_client):
    yield list(cluster_resource(CustomResourceDefinition)(admin_client))
...
def running_hco_containers(admin_client, hco_namespace):
    pods = []
    for pod in cluster_resource(Pod)(admin_client, namespace=hco_namespace.name):
...
def resource_type(request, admin_client):
    ...
    if not next(cluster_resource(resource_type)(admin_client), None):
