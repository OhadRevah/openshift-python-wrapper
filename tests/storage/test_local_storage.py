...
def local_storage_pv_spec(request, admin_client):
    for local_sc_pv in PersistentVolume.get(
        dyn_client=admin_client, label_selector=request.param["pv_label"]
    ):
...
def local_storage_profile_claim_property_sets(local_storage_class):
    return StorageProfile(name=local_storage_class.name).instance.status[
        "claimPropertySets"
    ][0]
...
def test_local_storage_profile_claim_property_sets(
    ...
    assert (
        local_storage_profile_claim_property_sets["accessModes"][0] == access_mode
    ), f"accessModes is not {access_mode} in storage class {local_storage_class.name}"
    assert (
        local_storage_profile_claim_property_sets["volumeMode"] == volume_mode
