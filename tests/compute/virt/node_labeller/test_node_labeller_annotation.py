    ...
    with ResourceEditor(
        patches={
            worker_node1: {
                "metadata": {
                    "annotations": {
                        f"{worker_node1.ApiGroup.NODE_LABELLER_KUBEVIRT_IO}/skip-node": "true"
                    }
                }
            }
        }
...
def worker1_supported_cpu_models_labels(worker_node1):
    node_cpu_models_labels = [
        ...
        if label_name.startswith(CPU_MODEL_LABEL_PREFIX)
        and label_value == ENABLED_CPU_LABEL_VALUE
    ]
    assert (
        node_cpu_models_labels
...
def labelled_worker_node1(worker1_supported_cpu_models_labels, worker_node1):
    ...
    with ResourceEditor(
        patches={
            worker_node1: {
                "metadata": {"labels": {updated_label: DISABLED_CPU_LABEL_VALUE}}
            }
        }
    ):
...
class TestNodeLabellerSkipAnnotation:
    ...
    def test_node_labeller_added_skip_node_annotation(
        ...
        LOGGER.info(
            f"Verify {label} on {node.name} is not reconciled after {int(RECONCILE_TIMEOUT/60)} minutes."
        )
        try:
            for sample in TimeoutSampler(
                ...
                if sample:
                    raise ResourceValueError(
                        f"Node {node.name} label {label} was reconciled"
                    )
    ...
    def test_node_labeller_removed_skip_node_annotation(self, labelled_worker_node1):
        ...
        try:
            ...
            raise_multiple_exceptions(
                exceptions=[
                    ResourceValueError(
                        f"Node {node.name} label {label} was not reconciled"
                    ),
                    exp,
                ]
            )
