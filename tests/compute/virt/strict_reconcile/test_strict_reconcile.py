...
ROLE_BINDING_SUBJECT = [
    {"kind": "ServiceAccount", "name": "default", "namespace": "openshift-cnv"}
]
...
def update_resource_and_prepare_sampler(resource, resource_dict):
    ...
    LOGGER.info(
        f"Waiting for resource {resource.kind}: {resource.name} to be reconciled."
    )
...
def restore_and_log_error(
    ...
    LOGGER.error(f"Expected: {expected_value} " f"Actual: {actual_value}")
    raise


def verify_resource_reconciled(resource):
    if resource.kind == ConfigMap.kind:
        verify_reconciled_configmap_resource(
            resource=resource, resource_dict={"data": CM_DATA}
        )
    elif resource.kind == Secret.kind:
        verify_reconciled_secret_resource(
            resource=resource, resource_dict={"data": SECRET_DATA}
        )
        ...
        verify_reconciled_role_or_clusterrole_resource(
...
def verify_reconciled_rolebinding_or_clusterrolebinding_resource(
    ...
    try:
        for sample in updated_and_reconciled_resource["samples"]:
            ...
            if (
                ...
                and original_resource_dict["subjects"][0]["name"]
                == sample_subjects[0]["name"]
            ):
                break
    except TimeoutExpiredError:
        restore_and_log_error(
...
def test_strict_reconcile_resources(
    admin_client, hco_namespace, resource_type, managed_resource_name
):
    ...
    for resource in resource_type.get(
        ...
        verify_resource_reconciled(resource=resource)
