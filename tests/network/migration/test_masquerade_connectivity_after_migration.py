...
@pytest.fixture
def migrated_vm_for_migration(running_vm_for_migration):
    LOGGER.info(
    ...
    migrated_vm = migrate_vm_and_verify(
...
def test_connectivity_after_migration(
    ...
    migrated_vm_for_migration,
    vm_console_connection_ready,
):
    """
    test for connectivity of a migrated vm with masquerade.
    using console to ping from migrated_vm_for_migration to running_vm_static.
