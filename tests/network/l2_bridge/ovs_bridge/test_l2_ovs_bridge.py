...
CUSTOM_ETH_PROTOCOL = "0x88B6"  # rfc5342 Local Experimental Ethertype. Used to test custom eth type and OVS bridge
...
class TestL2OVSBridge:
    """
    Test L2 connectivity via OVS bridge CNI plugin.
    The main goal is to make sure that different kinds of L2 traffic can pass
    transparently via OVS Bridge.
    """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                marks=(pytest.mark.polarion("CNV-2906")),
            ),
        ],
        ids=["dot1q", "mpls"],
    )
    ...
    @pytest.mark.polarion("CNV-2907")
    def test_dhcp_broadcast(self, configured_vm_a, configured_vm_b, dhcp_nad):
        """
        Test broadcast traffic via L2 OVS bridge. VM_A has dhcp server installed. VM_B dhcp client.
        """
    ...
    @pytest.mark.polarion("CNV-2909")
    def test_icmp_multicast(self, namespace, configured_vm_a, configured_vm_b):
        """
        Test multicast traffic(ICMP) via OVS bridge
        """
