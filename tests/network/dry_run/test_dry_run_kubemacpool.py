...
def dry_run_vma_mac_address(dry_run_vma):
    vm_mac_address = dry_run_vma.spec.template.spec.domain.devices.interfaces[1][
        MAC_ADDRESS
    ]
    # dry-run VM's have only "spec" or "metadata" attributes
    LOGGER.info(
        f"Allocated MAC address {vm_mac_address} for {dry_run_vma.metadata.name}"
    )
...
def running_vm_mac_addresses(vm_with_mac_address):
    ...
    for interface in vm_interfaces:
        LOGGER.info(
            f"interface_name {interface['name']} was allocated with MAC address {interface[MAC_ADDRESS]}"
        )
