...
class TestL2LinuxBridge:
    ...
    @pytest.mark.parametrize(
        "dst_ip",
        [
            pytest.param("configured_vm_b.dot1q_ip", marks=(pytest.mark.polarion("CNV-2277"))),
            pytest.param("configured_vm_b.mpls_local_ip", marks=(pytest.mark.polarion("CNV-2285")))],
        ids=[
            "dot1q",
            "mpls"
        ]
    ...
    def test_connectivity_l2_bridge(
        ...
        run_test_connectivity(
            src_vm=configured_vm_a.name, dst_vm=configured_vm_b.name,
            dst_ip=eval(dst_ip), positive=True, namespace=namespace.name
        )
    ...
    def test_dhcp_broadcast(
        ...
        current_ip = TimeoutSampler(
            timeout=60, sleep=2, func=get_vmi_ip_by_name,
            vmi=configured_vm_b.vmi, name=dhcp_nad.name
        )
    ...
    def test_custom_eth_type(
        ...
        with _open_console(configured_vm_b) as vmb_console:
            ...
            with _open_console(configured_vm_a) as vma_console:
                ...
                vma_console.expect('[1]')  #: Expected output. This is OK signal from nping
